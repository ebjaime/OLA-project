...
for e in tqdm(range(0,n_experiments)):  # cycle on experiments
  ...
  for t in range(0,T):  # cycle on time horizon
    ...
    # GPTS
    pulled_bid_ts = gpts_learner.pull_arm(curr_opt_gpts)
    ...
    for k in range(reward_click_ts):
      pulled_price , curr_opt_gpts = pricing_learner_gpts.pull_arm()
    ...
    # GPUCB
    pulled_bid_ucb = gpucb_learner.pull_arm(curr_opt_gpucb)
    ...
    for k in range(reward_click_ucb):
      pulled_price, curr_opt_gpucb = pricing_learner_gpucb.pull_arm()
      reward_price = env.pricing_round(pulled_price)
...
#np.savetxt('part3_gpts.txt', gpts_rewards_per_experiment)
#np.savetxt('part3_gpucb.txt', gpucb_rewards_per_experiment)
