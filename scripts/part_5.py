...
"""## Optimizing hyper-parameters (CD)"""

#pip install hyperopt on your environment
from hyperopt import fmin, tpe, hp

n_evals = 200
n_exp = 5
time_horizon_estimate = 30050

def objective(params):
    M = int(params['M'])
    eps = params['eps']
    h = np.log(time_horizon_estimate)*params['h']
    alpha = np.sqrt(np.log(time_horizon_estimate)/time_horizon_estimate)*params['alpha']
    ...
    for e in range(n_exp):
        ...
        cusum_rewards = cusum_daily_pricing_reward - cusum_collected_costs

        average_reward += -np.sum(cusum_rewards)/n_exp

    return average_reward # total reward
...
space ={
    'M': hp.loguniform('M', 4.6, 8),       # Range: 100 to 3000 (integer values) (log scale)
    'eps': hp.loguniform('eps', -20, 2.3),       # Range: 1e-9 to 10 (log scale)
    'h': hp.loguniform('h', -2.3, 2.3),            # Range: 0.1 to 10 (log scale)
    'alpha': hp.loguniform('alpha', -2.3, 2.3)     # Range: 0.01 to 10 (log scale)
}
# Run the hyperparameter optimization to retrieve optimal variables for CUSUM algorithm
best = fmin(objective, space, algo=tpe.suggest, max_evals=n_evals)
# Retrieve the best hyperparameters
best_M = int(best['M'])
best_eps = best['eps']
best_h = np.log(time_horizon_estimate)*best['h']
best_alpha = np.sqrt(np.log(time_horizon_estimate)/time_horizon_estimate)*best['alpha']

#optimal parameters for CUSUM algorithm
best_M, best_eps, best_h, best_alpha

"""## Optimizing hyper-parameters (SW)"""
...
def objective(params):
    ...
    for e in range(n_exp):
        ...
        sw_ucb_rewards = sw_ucb_daily_pricing_reward - sw_ucb_collected_costs

        average_reward += -np.sum(sw_ucb_rewards)/n_exp
...
# Define the search space for hyperparameters
space ={
    'tau': hp.loguniform('tau', 4.6, 9.2),       # Range: 100 to 10000 (integer values) (log scale)
}
# Run the hyperparameter optimization to retrieve optimal variables for CUSUM algorithm
best = fmin(objective, space, algo=tpe.suggest, max_evals=n_evals)
...
for e in tqdm(range(0,n_experiments)):  # cycle on experiments
  ...
  for t in range(0,T):  # cycle on time horizon
    ...
    ts_daily_pricing_reward = np.append(ts_daily_pricing_reward,
                                        (reward_click>0)*np.sum(ts_learner.collected_rewards[-reward_click:]))
    ...
    ucb_daily_pricing_reward = np.append(ucb_daily_pricing_reward,
                                         (reward_click>0)*np.sum(ucb_learner.collected_rewards[-reward_click:]))
...
#     print(cusum_learner.collected_rewards[-reward_click:])
  ...
  rounds = np.append(rounds,ts_learner.t)
  rounds = np.append(rounds,ucb_learner.t)
...
np.mean(cusum_numofchanges_per_experiment)

print(np.mean(rounds))
print(np.sqrt(np.var(rounds)))
...
# plt.legend(["TS", "UCB", "CUSUM","SW"])
plt.legend(["UCB", "CUSUM","SW"])
